...
const NEPAL_LOGO_VIEWBOX = "0 0 533 94";

type NepalLogoSize = "nav" | "footer" | "hero";
type NepalLogoVariant = "slot";
...
const frameWidthBySize = {
  nav: "150px",
  footer: "182px",
  hero: "clamp(196px, 28vw, 250px)",
} as const satisfies Record<NepalLogoSize, string>;

const svgWidthBySize = {
  nav: "126px",
  footer: "156px",
  hero: "clamp(172px, 24vw, 220px)",
} as const satisfies Record<NepalLogoSize, string>;

const framePaddingXBySize = {
  nav: "10px",
  footer: "11px",
  hero: "14px",
} as const satisfies Record<NepalLogoSize, string>;

const framePaddingYBySize = {
  nav: "8px",
  footer: "9px",
  hero: "12px",
} as const satisfies Record<NepalLogoSize, string>;
  ...
  return (
    <span
      className={joinClasses(
        "nepal-logo",
        `nepal-logo--${variant}`,
        glow && "nepal-logo--glow",
        className,
      )}
      style={
        {
          "--logo-frame-width": frameWidthBySize[size],
          "--logo-svg-width": svgWidthBySize[size],
          "--logo-frame-pad-x": framePaddingXBySize[size],
          "--logo-frame-pad-y": framePaddingYBySize[size],
        } as CSSProperties
      }
    >
      <span aria-hidden="true" className="nepal-logo__shine" />

      <svg
        aria-hidden={decorative}
        aria-label={decorative ? undefined : ariaLabel}
        className="nepal-logo__svg"
        focusable="false"
        preserveAspectRatio="xMidYMid meet"
        role={decorative ? undefined : "img"}
        shapeRendering="geometricPrecision"
        viewBox={NEPAL_LOGO_VIEWBOX}
      ...
      >
        {letters.map((letter, index) => (
          <g
            ...
            className={joinClasses(
              "nepal-logo__letter",
              `nepal-logo__letter--${letter.classSuffix}`,
              `nepal-logo__letter--${letter.tone}`,
            )}
            data-letter={letter.dataLetter}
            data-tone={letter.tone}
            style={
              {
                "--logo-letter-delay": `${index * 90}ms`,
